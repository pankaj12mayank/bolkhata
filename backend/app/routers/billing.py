...
def _effective_limit(shop: models.Shop, settings: models.PlatformSettings) -> int:
    # Free tier uses settings.free_entries_limit unless shop has override
    # If Paid and paid_entries_limit == -1 => unlimited (return large)
...
def get_billing(shop: models.Shop = Depends(get_current_shop), db: Session = Depends(get_db)):
    ...
    if shop.plan_tier == "Free" and shop.entries_limit != ps.free_entries_limit:
        ...
        db.commit()
    return {"tier": shop.plan_tier, "used": shop.entries_used_this_month, "limit": limit, "price": ps.paid_price_inr}


@router.post("/create-order", response_model=schemas.BillingOrderOut)
def create_order(shop: models.Shop = Depends(get_current_shop), db: Session = Depends(get_db)):
    """Create Razorpay order. If keys not configured, returns mock order. Frontend checks mock flag."""
    ps = get_settings(db)
    _check_monthly_reset(shop, db)
    if shop.plan_tier == "Paid":
        raise HTTPException(status_code=400, detail="Aap already Paid plan pe hain")

    amount = ps.paid_price_inr
    if amount <= 0:
...
def verify_payment(payload: schemas.BillingVerifyIn, shop: models.Shop = Depends(get_current_shop), db: Session = Depends(get_db)):
    ...
    if is_mock:
        ...
        db.commit()
        db.refresh(shop)
        return {"tier": shop.plan_tier, "used": shop.entries_used_this_month, "limit": _effective_limit(shop, ps), "price": ps.paid_price_inr}
...
def upgrade_plan(shop: models.Shop = Depends(get_current_shop), db: Session = Depends(get_db)):
    ...
    if not is_razorpay_configured(ps):
        ...
        db.add(models.Subscription(shop_id=shop.id, amount=ps.paid_price_inr, razorpay_id=mock_id, razorpay_order_id="order_mock_" + mock_id, status="Success"))
        db.commit()
        db.refresh(shop)
        return {"tier": shop.plan_tier, "used": shop.entries_used_this_month, "limit": _effective_limit(shop, ps), "price": ps.paid_price_inr}
    else:
