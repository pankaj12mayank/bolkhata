  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-7">
        <div className="bg-surface border border-line rounded-2xl p-4 sm:p-5"><div className="font-mono text-[24px] sm:text-[28px] font-semibold">{shop.customers_count}</div><div className="text-[12px] sm:text-[13px] text-ink-dim mt-0.5">{t('sd_customers')}</div></div>
        <div className="bg-surface border border-line rounded-2xl p-4 sm:p-5"><div className="font-mono text-[24px] sm:text-[28px] font-semibold">{shop.entries_count}</div><div className="text-[12px] sm:text-[13px] text-ink-dim mt-0.5">{t('sd_entries')}</div></div>
        <div className="bg-surface border border-line rounded-2xl p-4 sm:p-5"><div className="font-mono text-[22px] sm:text-[28px] font-semibold truncate">{fmt(shop.outstanding)}</div><div className="text-[12px] sm:text-[13px] text-ink-dim mt-0.5">{t('sd_outstanding')}</div></div>
        <div className="bg-surface border border-line rounded-2xl p-4 sm:p-5"><div className="font-mono text-[16px] sm:text-[18px] font-semibold">{new Date(shop.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</div><div className="text-[12px] sm:text-[13px] text-ink-dim mt-0.5">{t('sd_joined')}</div></div>
      </div>
      ...
      <GlobalPopup
        ...
        message={popup.type === "delete" ? t('sd_popup_delete_m', { name: shop.shop_name }) : t('sd_popup_toggle_m', { name: shop.shop_name, action: shop.status === "Active" ? t('sh_popup_action_deactivated') : t('sh_popup_action_activated') })}
        confirmText={popup.type === "delete" ? t('sh_popup_delete_c') : t('sd_popup_toggle_c')}
